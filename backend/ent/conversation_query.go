...
func (cq *ConversationQuery) QueryPrompt() *PromptQuery {
	query := (&PromptClient{config: cq.config}).Query()
...
func (cq *ConversationQuery) Clone() *ConversationQuery {
	...
	return &ConversationQuery{
		...
		withPrompt: cq.withPrompt.Clone(),
...
func (cq *ConversationQuery) WithPrompt(opts ...func(*PromptQuery)) *ConversationQuery {
	query := (&PromptClient{config: cq.config}).Query()
...
//		GroupBy(conversation.FieldMessages).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (cq *ConversationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Conversation, error) {
	...
	if query := cq.withPrompt; query != nil {
		if err := cq.loadPrompt(ctx, query, nodes, nil,
			func(n *Conversation, e *Prompt) { n.Edges.Prompt = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ConversationQuery) loadPrompt(ctx context.Context, query *PromptQuery, nodes []*Conversation, init func(*Conversation), assign func(*Conversation, *Prompt)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Conversation)
	for i := range nodes {
		if nodes[i].prompt_conversations == nil {
			continue
		}
		fk := *nodes[i].prompt_conversations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "prompt_conversations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
