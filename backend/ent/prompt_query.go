...
func (pq *PromptQuery) QueryConversations() *ConversationQuery {
	query := (&ConversationClient{config: pq.config}).Query()
...
func (pq *PromptQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (pq *PromptQuery) Clone() *PromptQuery {
	...
	return &PromptQuery{
		...
		withConversations: pq.withConversations.Clone(),
...
func (pq *PromptQuery) WithConversations(opts ...func(*ConversationQuery)) *PromptQuery {
	query := (&ConversationClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withConversations = query
	return pq
}
...
func (pq *PromptQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Prompt, error) {
	...
	if query := pq.withConversations; query != nil {
		if err := pq.loadConversations(ctx, query, nodes,
			func(n *Prompt) { n.Edges.Conversations = []*Conversation{} },
			func(n *Prompt, e *Conversation) { n.Edges.Conversations = append(n.Edges.Conversations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PromptQuery) loadConversations(ctx context.Context, query *ConversationQuery, nodes []*Prompt, init func(*Prompt), assign func(*Prompt, *Conversation)) error {
	...
	query.Where(predicate.Conversation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(prompt.ConversationsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.prompt_conversations
		if fk == nil {
			return fmt.Errorf(`foreign-key "prompt_conversations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "prompt_conversations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
